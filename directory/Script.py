...
def Start():
    ...
    '''Body Context'''
    body = open("Emails/body.txt", 'rt')
    bodyConent = body.read()
    
    print(hcolors.GREEN + '[+] Body message will be sent from file "Emails/body.txt" !!')
    
    '''list of targets to send mails'''
    targets = open("Emails/target_list.txt", 'rt')
    target_context = targets.read()
    print('[+] Target lists taken from the file "Emails/target_list.txt" !!')
